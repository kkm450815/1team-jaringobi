...
- **현재 값**: `versionCode 13` / `versionName "1.0.10"` (2026-05-18)
  - 1.0.10 내용: 메인 헤더 원상 복구 — 1.0.9 에서 마이페이지를 룸 우측 상단으로 옮기고 헤더 자리에 설정 바로가기를 신설했었는데, 사용자 피드백으로 되돌림. 마이페이지는 헤더 우측 원래 자리로 복귀, 설정은 마이페이지 우상단 톱니바퀴 버튼으로 진입. 1.0.9 의 나머지 변경(절약 내역 모달·뒤로가기 고정·동요풍 2차 보강·관리자 기본 아이템 표시) 은 그대로 유지.
- 과거 이력:
