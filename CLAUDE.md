...
- **현재 값**: `versionCode 14` / `versionName "1.0.11"` (2026-05-18)
  - 1.0.11 내용: 동요풍 BGM 3차 보강 — sine 만으론 perceived loudness 한계라 triangle 레이어를 베이스/멜로디에 함께 섞어 배음 추가, 게인도 추가 상향(베이스 1.4→2.0 + 옥타브 위 0.55→0.9 + triangle 0.55, 멜로디 1.55→2.3 + 옥타브 위 0.7→1.2 + triangle 0.85, 5도 화음 0.5→0.85). 마이페이지 공유 캡쳐 우측 잘림 완전 수정 — 진짜 원인이 section 의 CSS box-shadow 가 scrollWidth 에 안 잡혀서였음. 캡쳐 동안 inline style 로 box-shadow 일시 제거 후 finally 에서 복원. 추가로 pixelRatio 2→1.5 다운(공유 파일 크기 축소 + sub-pixel 잘림 추가 방지), 우측 버퍼 32→64px, 하단 16→32px 상향.
- 과거 이력:
