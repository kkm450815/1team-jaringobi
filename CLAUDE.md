...
- **현재 값**: `versionCode 9` / `versionName "1.0.6"` (2026-05-15)
  - 1.0.6 내용: 마이페이지 칭호/공유 모달 UI 개선(상세→그리드 복귀, X 버튼 확대, 공유 미리보기 폭 축소), 닉네임 입력 비제어 전환, BGM 3종(차분/동요풍/8비트 레트로) 선택 + 마스터 게인 3.5 상향, 상점 장바구니 + 카테고리(슬롯)별 1개 동시 시착, 구글 로그인 단계별 에러 로그 보강
- 과거 이력:
