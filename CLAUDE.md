...
- **현재 값**: `versionCode 15` / `versionName "1.0.12"` (2026-05-18)
  - 1.0.12 내용: 상점 카드의 ✓/보유/자물쇠 뱃지가 스크롤 시 어긋나 보이던 문제 수정. 원인은 안드로이드 WebView 의 `<button>` 안 position:absolute 자식 컴포지팅 이슈. 옷장이 이미 쓰던 외부 div + 내부 button 패턴으로 통일하고 뱃지를 div 직접 자식 absolute 로 배치 + pointer-events-none 추가. 옷장은 이미 동일 패턴이라 수정 불필요.
- 과거 이력:
